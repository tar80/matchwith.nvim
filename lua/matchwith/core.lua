...
function Matchwith:new(is_insert_mode)
  ...
  Instance['word_highlight'] = vim.g.matchwith_word_highlight
  Instance['avoid_word_type'] = vim.g.matchwith_avoid_word_type
...
function Matchwith:search_cursor_word(tsroot, queries)
  ...
  if node_type ~= node_text and not vim.tbl_contains(self.avoid_word_type, node_type) then
    local iter_captures = queries:iter_captures(tsroot, self.bufnr, self.top_row, self.bottom_row + 1, {
  ...
  if #words > 0 then
    vim.iter(words):each(function(range)
      vim.api.nvim_buf_set_extmark(self.bufnr, Cache.ns_word, range[1], range[2], {
        end_row = range[3],
        end_col = range[4],
  ...
  for _, matches in iter_matches do
    for id, nodes in pairs(matches) do
      ...
      for _, node in ipairs(nodes) do
        if ts.is_contains(node, instance.cur_row, instance.cur_col) then
          match.word = node
        end
  ...
  if searchpair then
    ...
    if searchpair_opts then
      ...
      if (pair_pos[1] + pair_pos[2]) ~= 0 then
        ...
        local last_range = self.word_highlight and position.to_range4(self.cur_row, col)
  ...
  if next_range then
    ...
    if parent then
      ...
      if is_start_point or ts.is_contains(parent, self.cur_row, self.cur_col) then
        ...
        local last_range = self.word_highlight and position.to_range4(self.cur_row, self.cur_col) or next_range
        if is_next then
...
function Matchwith.matching(is_insert_mode)
  ...
  if Instance.is_insert_mode then
    ...
    Instance.word_highlight = false
  end

  -- use cache
  local last_range = Instance.word_highlight and Cache.last_word or Cache.last.range
  if (Instance.changetick == Cache.changetick) and ts.is_contains(last_range, Instance.cur_row, Instance.cur_col) then
    return
  end
  vim.api.nvim_buf_clear_namespace(Instance.bufnr, Cache.ns_word, 0, -1)
