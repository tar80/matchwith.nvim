...
function matchwith.clear_ns(self)
  local clear = false
  -- local col = self.is_insert_mode and self.cur_col + 1 or self.cur_col
  -- print(self.is_insert_mode, self.cur_col,col,cache.last.word[2],cache.last.word[3])
...
---@return Range4|nil
local function _find_sibling(is_start_point, parent, node_ranges)
  ---@type TSNode?,string
  local node, method
...
---@return string[]|nil
local function _get_searchpair_options(sentence, col)
  if vim.fn.type(sentence) ~= 10 then
...
function matchwith.get_matchpair(self)
  ...
  if parent then
    ...
    local pair_range = _find_sibling(is_start_point, parent, self.node_ranges)
    if pair_range then
...
function matchwith.draw_markers(self)
  ...
  if not self.is_insert_mode and (fn.foldclosed(self.cur_row + 1) == -1) then
    if num > 0 then
...
function matchwith.add_marker(self, hlgroup, word_range)
  ...
  local opts = {}
  vim.hl.range(self.bufnr, cache.ns, hlgroup, s, e, opts)
end
...
function matchwith.matching(is_insert_mode, row, col)
  ...
  -- print(session.cur_row, session.cur_col)
  ...
  if session.is_insert_mode then
    ...
    if session.cur_col < 0 then
      -- vim.print('cache.marker:',cache.marker)
...
function matchwith.setup(opts, force)
  ...
  if not hl then
    vim.notify('Requires arguments', vim.log.levels.ERROR, { title = UNIQUE_NAME })
    return
  end
